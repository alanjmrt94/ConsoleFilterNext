...
@Mod.EventBusSubscriber(modid = ConsoleFilterNeoForge.MODID)
public final class NeoForgeCommands {
		...
		return Commands.literal("consolefilter")
			...
			.then(Commands.literal("profile")
				.then(Commands.literal("default")
					.executes(ctx -> setProfile(ctx.getSource(), NeoForgeConsoleFilterConfig.PROFILE_DEFAULT)))
				.then(Commands.literal("debug")
					.executes(ctx -> setProfile(ctx.getSource(), NeoForgeConsoleFilterConfig.PROFILE_DEBUG)))
				.then(Commands.literal("production")
					.executes(ctx -> setProfile(ctx.getSource(), NeoForgeConsoleFilterConfig.PROFILE_PRODUCTION))));
	}

	private static ConsoleFilterNeoForge requireMod(CommandSourceStack source) {
		ConsoleFilterNeoForge mod = ConsoleFilterNeoForge.getInstance();
		if (mod == null) {
	...
	private static int reload(CommandSourceStack source) {
		ConsoleFilterNeoForge mod = requireMod(source);
		if (mod == null) {
			return 0;
		}
